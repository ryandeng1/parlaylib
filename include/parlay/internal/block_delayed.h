...
struct flatten_iterator {
  ...
  in_iter_t in_iter;
  out_iter_t out_iter;
  out_iter_t out_end;  // Track end of outer range to prevent segfault
  flatten_iterator& operator++() {
    ...
    while (in_iter == (*out_iter).end()) {// if at end of inner range
      ...
      if (out_iter == out_end) break; // Check bounds before dereferencing
...
struct block_delayed_sequence {
  ...
  iterator begin() {return rng.begin();}
  iterator end() {return rng.end();}
  block_delayed_sequence(parlay::sequence<IDS> sub_ranges_, size_t n)
      : sub_ranges(std::move(sub_ranges_)),
        rng(stream_delayed::forward_delayed_sequence(flatten_iterator(sub_ranges.begin(), sub_ranges.end()),n)) {}
...
auto flatten(Seq &seq) {
  ...
  auto results = internal::tabulate(num_blocks, [&, block_size=block_size] (size_t i) {
    ...
    return stream_delayed::forward_delayed_sequence(flatten_iterator(in_iter, out_iter, seq.end()), len);
  }, 1);
  return block_delayed_sequence(std::move(results), n);
}
